...
export default function HistoryPage() {
  ...
  const filteredDeliveries = useMemo(() => {
    ...
    if (statusFilter === "delivered") {
      ...
      result = result.filter(
        (d) =>
          !["delivered", "completed", "cancelled", "failed"].includes(d.status),
      );
    }
  ...
  const fetchHistory = useCallback(async () => {
    ...
    if (error) {
      ...
      // Calculate stats
      const delivered = deliveryList.filter(
        (d) => d.status === "delivered" || d.status === "completed",
      );
      const cancelled = deliveryList.filter(
        (d) => d.status === "cancelled" || d.status === "failed",
      );
      ...
      setStats({
        ...
        totalEarnings: delivered.reduce(
          (sum, d) => sum + (d.driver_earnings || 0),
          0,
        ),
        totalDistance: delivered.reduce(
          (sum, d) => sum + (Number(d.distance_km) || 0),
          0,
        ),
      });
    }
  ...
  const loadMore = useCallback(async () => {
    ...
    if (error) {
      ...
      if (newItems.length > 0) {
        setStats((prev) => {
          const delivered = newItems.filter(
            (d) => d.status === "delivered" || d.status === "completed",
          );
          const cancelled = newItems.filter(
            (d) => d.status === "cancelled" || d.status === "failed",
          );
          return {
            ...
            totalEarnings:
              prev.totalEarnings +
              delivered.reduce((sum, d) => sum + (d.driver_earnings || 0), 0),
            totalDistance:
              prev.totalDistance +
              delivered.reduce(
                (sum, d) => sum + (Number(d.distance_km) || 0),
        ...
        {!loading && hasActiveFilters && (
          <div className="px-3 pt-1 pb-0 flex items-center justify-between">
            <p className="text-xs text-gray-500">
              {filteredDeliveries.length} livraison
