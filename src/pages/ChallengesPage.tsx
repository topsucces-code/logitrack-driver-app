...
export default function ChallengesPage() {
  ...
  const handleClaimReward = async (challenge: Challenge) => {
    ...
    if (result.success) {
      ...
      setChallenges((prev) =>
        prev.map((c) =>
          c.id === challenge.id ? { ...c, rewardClaimed: true } : c,
        ),
      );
    } else {
        ...
        {stats && (
          <div className="bg-white/10 rounded-lg p-2">
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-xs text-white/70">
                {stats.xpToNextLevel.toLocaleString()} {t.xp} →
              </span>
            </div>
            <div className="h-1.5 bg-white/20 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: `${Math.min(100, (stats.xp / (stats.xp + stats.xpToNextLevel)) * 100)}%`,
                }}
              />
            </div>
          </div>
          ...
          <>
            ...
            {activeTab === "badges" && (
              <div className="space-y-3">
                {["deliveries", "earnings", "rating", "streak", "distance"].map(
                  (category) => {
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-center justify-between mb-1">
                                  ...
                                  {isUnlocked ? (
                                    <div
                                      className={`w-5 h-5 rounded-full bg-gradient-to-br ${tierColors[badge.tier]} flex items-center justify-center`}
                                    >
                                ...
                                <p
                                  className={`font-medium text-xs ${isUnlocked ? "text-gray-900 dark:text-white" : "text-gray-500"}`}
                                >
                                  {badge.name}
                                </p>
