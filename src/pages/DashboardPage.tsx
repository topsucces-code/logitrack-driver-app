    ...
    try {
      ...
      if (weekDeliveries) {
        setWeekStats({
          ...
          earnings: weekDeliveries.reduce(
            (sum, d) => sum + (d.driver_earnings || 0),
            0,
          ),
        });
    ...
    const channel = subscribeToNotifications(
      ...
      (notif: AppNotification) => {
        ...
        if (
          ...
          notif.template === "delivery_assigned"
        ) {
          fetchDeliveries();
  ...
  const acceptDelivery = useCallback(
    async (deliveryId: string) => {
      ...
      if (!error && data?.success) {
        ...
        showError(
          error?.message || data?.error || "Erreur lors de l'acceptation",
        );
      }
    },
    ...
    <div className="h-mobile-screen flex flex-col bg-gray-50 dark:bg-gray-900">
      ...
      <header className="bg-primary-500 text-white safe-top px-3 pt-3 pb-3">
        ...
        <button
          ...
          aria-label={
            isOnline
              ? "En ligne - cliquez pour passer hors ligne"
              : "Hors ligne - cliquez pour passer en ligne"
          }
        ...
        >
          <div className="flex items-center gap-2">
            <div
              className={`w-2.5 h-2.5 rounded-full ${isOnline ? "bg-white animate-pulse" : "bg-white/50"}`}
            />
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span className="px-2.5 py-0.5 bg-primary-500 text-white text-[10px] font-medium rounded-full">
                    ...
                    {currentDelivery.status === "picking_up" &&
                      "🚀 En route pickup"}
                ...
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <p className="font-medium text-gray-900 text-xs truncate">
                    {currentDelivery.pickup_contact_name ||
                      currentDelivery.vendor_name ||
                      "Pickup"}
                  </p>
                </div>
                ...
                <div className="flex items-center justify-between pt-2 border-t border-primary-200">
                  ...
                  <p className="font-bold text-primary-600 text-base">
                    {(
                      currentDelivery.driver_earnings ||
                      currentDelivery.total_price ||
                      0
                    ).toLocaleString()}{" "}
                    F
                  </p>
          ...
          {!isVerified && (
            <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
              ...
              <p className="text-xs text-yellow-700 mt-1">
                Votre compte est en cours de vérification. Vous pourrez accepter
                des courses une fois vérifié.
              </p>
            </div>
          )}
        </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {timeLeft && (
          <div
            className={`flex items-center gap-1.5 text-xs mb-2.5 ${expired ? "text-red-600" : "text-orange-600"}`}
          >
